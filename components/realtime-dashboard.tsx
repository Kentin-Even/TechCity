...
import { useSensorStream } from "@/lib/hooks/use-sensor-stream";
import {
  Card,
  CardContent,
  CardDescription,
      ...
      <CardHeader>
        ...
        <CardDescription>
          {connectionError ? (
            ...
            "Données des capteurs IoT en temps réel"
          )}
        </CardDescription>
      </CardHeader>
      <CardContent>
...
export default function RealtimeDashboard() {
  ...
  const {
    ...
    onConnect: () => {
      console.log("🎉 Dashboard connecté au stream");
    },
    onDisconnect: () => {
      console.log("👋 Dashboard déconnecté du stream");
    },
    onData: (data) => {
      console.log("📊 Nouvelles données reçues:", data.length, "points");
    },
    onError: (error) => {
      console.error("❌ Erreur SSE dans dashboard:", error);
    },
  });
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold">Dashboard IoT Temps Réel</h1>
        ...
        <div className="flex gap-2">
          <button
            onClick={() => {
              console.log("🔘 Bouton cliqué, état actuel:", {
                isConnected,
                isConnecting,
              });
              if (isConnected) {
